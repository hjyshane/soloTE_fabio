...
# SoloTE list
# read
obj_sub <- qs::qread(file.path(directory[["qsave_dir"]], "03__final_locustes.qs"))
...
# test
# subfamily
solote_features_sub <- rownames(obj_sub)[grep(pattern = "^SoloTE-", rownames(obj_sub))]
...
# save
readr::write_csv(markers, file = file.path(directory[["markers_dir"]], "all_genes-locustes_withM_zeroM.csv"))
...
# save
readr::write_csv(sigmarkers, file = file.path(directory[["markers_dir"]], "sig_gene-locustes.csv"))
readr::write_csv(sigtemarkers, file = file.path(directory[["markers_dir"]], "sig_te_gene-locustes_NSC-NPC-DG.csv"))
...
ggplot2::ggsave(a, filename = file.path(directory[["plot_dir"]], "volcano_locustes.png"), width = 24, height = 12, dpi = 300, bg = "white")
